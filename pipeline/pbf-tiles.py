...
Aarhus: 3 x 3 road tiles plus the Letbane, which runs on tram track in the
city and on the old Grenaa and Odder railway lines outside it. The rail box reaches
further east than the roads on purpose: letbane L1 ends in Grenaa, 60 km out.
"""
import json, os, re, sys
import osmium

ROOT = os.path.join(os.path.dirname(__file__), '..')
PBFS = [os.path.join(ROOT, 'data', 'denmark-latest.osm.pbf')]

# must match pipeline/download.sh
S, N, W, E = 55.88, 56.50, 9.50, 10.72
RAIL_BOX = (55.95, 10.08, 56.46, 10.94)
...
RAIL = re.compile(r'^(tram|light_rail|rail|construction)$')
...
rail_file = os.path.join(ROOT, 'data/osm/aarhus-rail.json')
...
GEN = 'pbf-tiles.py (Geofabrik denmark)'
