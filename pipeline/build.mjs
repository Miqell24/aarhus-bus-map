...
// Aarhus: the NATIONAL Rejseplanen feed covers all of Denmark, so this map's
// scope is a precomputed allowlist (scope.mjs → data/scope.json) — Midttrafik's
// 59 city and regional bus lines within 25 km of Aarhus H, plus both Aarhus
// Letbane lines whole, L1 out to Grenaa and L2 down to Odder.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|L1,L2]
...
// Exact-match repairs for poles a feed names badly. Rejseplanen needs none —
// it is one of the best-cased feeds in this family.
const NAME_FIX = {};
...
// ONE national feed, TWO cfgs.
//
// The same Rejseplanen bundle that feeds copenhagen-bus-map, cut to Aarhus by
// pipeline/scope.mjs. Everything here is Midttrafik: route_type 3 the buses,
// 0 the letbane.
//
// Cut deliberately: route_type 715 — Flextur and the telebusser, 44 of them in
// this frame; they carry numbers but no fixed run (the Närtrafiken rule) — the
// DSB and GoCollective regional trains (2, as in Copenhagen) and the Samsø
// ferries (4; no water graph).
//
// The letbane keeps the family's tram red rather than Midttrafik's own line
// colours: colour means the MODE in this family, and Copenhagen's Hovedstadens
// Letbane set that precedent for light rail on its own alignment.
const LBL = new Map();
...
const S_BUS = new Set(SCOPE.bus), S_LET = new Set(SCOPE.letbane);

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Aarhus and its opland — 3 × 3 tiles
  osmFiles: Array.from({ length: 9 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['1A']),
  feeds: [
    { tag: 'mt', dir: 'data/gtfs', routeTypes: ['3'],
      skipRoute: (r) => !S_BUS.has(r.route_id), mapKey: (sn) => sn || null },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramSel.length) MODES.push({
  // L1 rides the old Grenaa railway and L2 the Odder one, so this graph needs
  // mainline rail as well as the street track in the city centre
  mode: 'tram', label: 'letbane', osmFile: 'data/osm/aarhus-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail', 'rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'mt', dir: 'data/gtfs', routeTypes: ['0'],
      skipRoute: (r) => !S_LET.has(r.route_id), mapKey: (sn) => sn || null },
  ],
});
